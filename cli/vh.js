...
function usage() {
  return [
    ...
    "  vh evidence verify-signed <signed> [--dir <d>] [--signer <0xaddr>] [--strict] [--revocations <f> --as-of <ISO>]  OFFLINE/key-free/network-free: RECOVER the signer from a signed evidence packet (Check 1, always), (--signer) PIN it to an expected signer, (--dir) BIND the signature to YOUR bytes, (--revocations) check the signer was not REVOKED as of --as-of (default now); leads with the trust caveat + prints per-check PASS/FAIL. A forged/tampered/wrong-key signature, or a key revoked-before-as-of, is a clean REJECTED/REVOKED — never a silent pass. WITHOUT --signer the verdict is labelled UNPINNED (anyone's key passes an unpinned check); --strict fails closed on that with the distinct exit 4. Exit 0 ACCEPTED (and pinned under --strict) / 3 REJECTED|REVOKED / 4 UNPINNED (--strict only) / 2 usage / 1 IO",
    ...
    "  vh agent coverage --repo <dir> --range <rev-range> --packets <dir> [--deep] [--require-all] [--require-since <oid>] [--out <report>] [--json]  the FLEET gate (T-71.2): enumerate the range's commits OLDEST-FIRST (git rev-list --reverse), FULLY verify every *.vhagent.json under --packets through the SAME verify path as `vh agent verify` (an unverifiable packet's claims count ONLY as claim-unverified-packet — never coverage — and the packet is NAMED in the report), extract the disclosed commit-claims, and report each commit from the CLOSED vocabulary covered-verified / covered-oid-only / claim-root-mismatch / claim-unverified-packet / uncovered. --deep re-derives each claimed commit's tracked-set root (the `vh hash --git` engine, hashGit) in a throwaway LOCAL clone — offline, removed on every exit path — so a lying gitRoot is the NAMED claim-root-mismatch; without --deep roots are NOT re-derived (covered-oid-only, and the output says so). Report-only default exits 0; --require-all / --require-since <oid> gate exit 3 when a required commit lacks a verifiable claim; --out writes the canonical, byte-diffable, SEALABLE vh-agent-coverage@1 report (seal it with `vh evidence seal`). An INVENTORY control, not an authorship detector — containment, NOT causation. FREE, read-only, key-less. CI recipes: verifier/ci/agent-coverage.generic.sh + verifier/ci/agent-coverage.github-actions.yml. Exit 0 pass|report-only / 3 gate-fail / 2 usage (incl. unknown --range) / 1 IO",
    ...
    "  the human-owned signing/timestamp trust-root (P-3, docs/TRUST-BOUNDARIES.md#p-3-trust-root). Exit 0; usage error 2; runtime 1.",
    "",
    ...
    "  signature is the human-owned signing/timestamp trust-root (P-3, docs/TRUST-BOUNDARIES.md#p-3-trust-root). Exit 0; usage 2; runtime 1.",
    "",
    ...
    "  signature is NOT a delivery timestamp (P-3, docs/TRUST-BOUNDARIES.md#p-3-trust-root). Exit 0 ACCEPTED, 3 REJECTED/REVOKED; usage 2; runtime 1.",
    "",
