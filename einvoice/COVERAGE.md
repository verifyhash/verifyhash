...
- **297 business rules** the engine actually asserts (this is the exact set the code fires — `test_coverage_matrix.py` proves it against the live registries).
- Syntax: **281** proven on both UBL and CII, **12** UBL-only, **4** CII-only.
- Severity (blocking class): **284** fatal (block validity), **13** warning / information (reported, non-blocking).
- **Official German messages: 50 rules** carry an official German assert message, surfaced by the CLI `--lang de` flag (`--json` and exit codes are unchanged). That text is lifted VERBATIM from the vendored KoSIT XRechnung Schematron (`de_source == "kosit"`), never machine-translated; where no official German assert text exists to quote, the assert message — and only the message — falls back to English. Rule titles and fix hints are German on all 297 catalogued rules, so `--lang de` renders the German headline and the `Behebung` hint (and `--explain` the same two fields) even on a finding whose message fell back. See the README `--lang de` section.
...
| `BR-CL-06` | UBL + CII | fatal | fatal | CEN EN 16931 1.3.16 | CEN EN 16931 1.3.16 | Value added tax point date code MUST be coded using a restriction of UNTDID 2005 (UBL) / UNTDID 2475 (CII). |
| `BR-CL-07` | UBL + CII | fatal | fatal | CEN EN 16931 1.3.16 | CEN EN 16931 1.3.16 | Object identifier identification scheme identifier MUST be coded using a restriction of UNTDID 1153. |
| `BR-CL-08` | UBL + CII | fatal | fatal | CEN EN 16931 1.3.16 | CEN EN 16931 1.3.16 | Invoice note subject code MUST be coded using UNTDID 4451. |
...
| `BR-CL-15` | UBL + CII | fatal | fatal | CEN EN 16931 1.3.16 | CEN EN 16931 1.3.16 | Item origin country codes MUST be coded using ISO 3166-1. |
...
| `BR-DE-22` | UBL + CII | fatal | fatal | KoSIT XRechnung 2.5.0 (XRechnung 3.0.2) | KoSIT XRechnung 2.5.0 (XRechnung 3.0.2) | The filename attribute of all EmbeddedDocumentBinaryObject elements must be unique (across cac:AdditionalDocumentReference). |
...
## French CIUS scope (EN 16931 core covered; FNFE-MPE / Chorus Pro rules not vendored)

This is a **technical** coverage statement, separate from the French *legislative* timeline in README §1 (reception mandatory September 2026, issuance phasing to September 2027 via the PDP / e-reporting model). France's B2B e-invoicing reform uses EN 16931 profiles: **Factur-X** (a CII payload in a PDF/A-3 container), UBL, and CII. Everything EN 16931 itself defines for such an invoice is already validated here:

- **The EN 16931 core applies to a French invoice today.** A French UBL or CII invoice is graded against the full EN 16931 `BR-*` semantic model documented in the table above — the identical core the German XRechnung profile builds on. Nothing French-specific is needed for the core rules to fire.
...
- Graded across **217 implemented core rules**; **64 distinct `BR-*` rules** are actually exercised by the CreditNote corpus (the rest have no CreditNote unit case to fire on).
...
## CII credit-note scope

Stated separately from the UBL CreditNote scope above. In CII there is **no separate credit-note root**: a credit note (e.g. a German Gutschrift) is the same `rsm:CrossIndustryInvoice` with BT-3 (`ram:ExchangedDocument/ram:TypeCode`) = **381**, so CII credit-note support is a property of the one CII engine, not a second document route. Measured and differentially proven 2026-07-17 (T-VHCNCII.1); pinned by `test_cii_creditnote.py`.

- **The official CII BR-CL-01 list is one MERGED invoice+credit-note list.** The vendored CEN EN16931-CII Schematron grades BT-3 against a single 62-code UNTDID 1001 list that contains 381 — and also 471/472/473/500/501, codes present in **neither** UBL sub-list — so the CII list is transcribed verbatim as its own set (`rules.UNTDID_1001_CII`), never derived as a union. `br_cl_01` branches on `inv.syntax`; the UBL Invoice and UBL CreditNote sub-lists are byte-untouched (381 remains invalid for a UBL `cbc:InvoiceTypeCode`).
- **Engine fix this measurement forced:** before T-VHCNCII.1 the shared `br_cl_01` graded every syntax against the UBL *Invoice* sub-list, so a valid CII 381 credit note wrongly fired BR-CL-01 (measured: OURS `{BR-CL-01}` vs OFFICIAL `{}` on the valid fixture). Fixed by exact transcription of the official CII binding — a correction *toward* the official artifact, not a loosening.
- **Committed fixtures (fully synthesized, fictional parties/IBAN): 2.** `fixtures/creditnote-valid_cii.xml` — a business-rule-clean 381 credit note (sha256 `26496955cc831dd4079a8f2ef7ff2bc76b88504b63e7836cf36f4d9efdf7db29`) — and `fixtures/creditnote-invalid_cii.xml` — the same document with BT-5 `ram:InvoiceCurrencyCode` removed (sha256 `6b31125ce20755400dc5ae13145b70963e0202b252e3d3c3f1dda1c5d9539bdc`).
- **Differential divergences on both fixtures: 0**, graded against the official vendored `EN16931-CII-validation.xslt` on BOTH EN16931-CII legs (core `CII_RULE_IDS` × `cii_our_fired` and syntax-binding `SB_CII_RULE_IDS` × `sb_cii_our_fired`): on the valid fixture official and ours both fire **nothing**; on the broken fixture both fire **exactly `BR-05`**. After the engine fix the full corpus legs were re-run on disk — `differential.py cii` and `differential.py sbcii`, **0 divergences each** — so the fix regressed nothing.
- **The raw-XML CLI grades these fixtures too, identically to the container path.** A raw CII `.xml` file and the same bytes embedded in a Factur-X/ZUGFeRD PDF now reach the CII engine through ONE dispatch seam (`validate.validate_root`), so the two routes cannot answer differently. Measured: `einvoice validate fixtures/creditnote-valid_cii.xml` exits 0 with `PASS:`, `einvoice validate fixtures/creditnote-invalid_cii.xml` exits 1 naming exactly `BR-05`, in text and `--json` form. Until 0.2.7 the raw-XML surface answered both files with a structural `S-ROOT` fatal instead; `S-ROOT` is now reserved for a root that is neither a UBL `Invoice`/`CreditNote` nor a CII `CrossIndustryInvoice`. Pinned by `test_cli_cii_root.py`.
- **Not proven / not claimed:** no XRechnung-CIUS (KoSIT) claim is made for the 381 shape — the fixtures declare `urn:cen.eu:en16931:2017` and the XRechnung-CII legs were not part of this proof.
...
Deliberate exclusions counted against each universe (4 ids, all
documented with reasons in the Exclusions section above): `BR-CO-05`, `BR-CO-06`, `BR-CO-07`, `BR-CO-08`.

### `en16931-ubl` — 219 implemented + 4 excluded + 0 missing = 223 official `BR-*` rules
...
### `en16931-cii` — 219 implemented + 4 excluded + 0 missing = 223 official `BR-*` rules
...
**TERMINAL — the CII proof-parity worklist is CLOSED.** Of the **297**
business rules the engine asserts, **281** are differentially proven
...
## Syntax-binding assert

Distinct from the scoped-out mentions above, this section is the live,
machine-recomputed accounting of the **syntax-binding** asserts the two
vendored preprocessed CEN artifacts carry ALONGSIDE the `BR-*` business
rules — the `sch:assert`s whose id is `UBL-CR-*` / `UBL-DT-*` /
`UBL-SR-*` (UBL) and `CII-DT-*` / `CII-SR-*` (CII). They are syntax-layer
restrictions — "this element MUST NOT appear", "at most one of X", a
decimal-place cap — not EN 16931 business rules.
...
- Honesty note: these are **NOT** part of the **297 business rules**
  ...
  class is now partially EVALUATED by a restricted, data-driven engine
  (`einvoice/syntax_binding_eval.py`) and surfaced under the distinct
  `syntax-binding` report category — the accounting is below, kept
  strictly separate from the business-rule count.

### UBL absence-restriction — implemented vs known-open

The dominant UBL class (`absence-restriction`, **699** of the 756 UBL syntax-binding asserts) is evaluated by a *restricted*, data-driven engine — NOT a general XPath processor. An assert is IMPLEMENTED iff its rule context is the document root (`/ubl:Invoice | /cn:CreditNote`) AND its `@test` matches the closed grammar the evaluator can prove equivalent to the official Schematron: a bare `not(<path>)` presence restriction, or the disjunctive `not(<path>) or <path> = '<literal>'`, over a location path of element steps (with `//` descendant search, union groups `(a|b)`, and a trailing `@attr`) — no predicates, no functions. Anything else is left known-open.

- **694 implemented** (differential-proven) / **5 known-open** of the **699** UBL `absence-restriction` asserts.
- Every implemented id is proven equivalent to the official CEN EN16931-UBL Schematron with **0 divergences** over the differential corpus + targeted fixtures (`differential.py` LEG 5, the `sb` leg).
- Findings surface under the distinct **`syntax-binding`** report category, each mirroring the official `@flag`: `warning` findings are reported but do NOT change the exit code; a `fatal` finding blocks validity. (All 694 implemented ids carry `warning`; the 3 `fatal` asserts in this class are all in the known-open set below.)
...
| id | @test (unsupported form) |
| --- | --- |
| `UBL-CR-412` | `not(cac:PaymentMeans/cbc:PaymentDueDate) or ../cn:CreditNote` |
| `UBL-CR-665` | `not(//cac:AdditionalDocumentReference[cbc:DocumentTypeCode != '130' or not(cbc:DocumentTypeCode)]/cbc:ID/@schemeID)` |
...
A `cardinality-count` assert caps how many times a repeatable UBL element may appear under a rule context, e.g. UBL-SR-01 `count(cac:ContractDocumentReference/cbc:ID) <= 1`. An assert is IMPLEMENTED iff BOTH its rule context compiles under the closed context-pattern grammar (a `|`-union of element-step chains — the document root, a `//` descendant, or a relative `a/b` path — with NO predicate `[...]`, function, or `@attr` step) AND its `@test` compiles under the closed count grammar: `count(P) OP n`, `not(P) or count(P) OP n`, the difference-of-counts `count(P1) - count(P2) OP n` (UBL-DT-18), or the `and`-conjoined `(count(P) OP n) and ((A) != (B))` (UBL-SR-19/21) — over restricted element paths (`A`/`B` may lead with a `..` parent step; the `!=` is the XPath general node-set inequality: ∃ a∈A, b∈B with different string values). A predicated / `upper-case()` / `preceding::` count path (UBL-SR-04/12/13/18/29/44/47), a predicated context (UBL-SR-30/31), or an `and`-conjoined comparison whose count path carries an `upper-case(@schemeID)` predicate (UBL-SR-20) is left known-open — never approximated.

- **42 implemented** (differential-proven) / **10 known-open** of the **52** UBL `cardinality-count` asserts (histogram total **52**).
...
- **4 implemented** (differential-proven) / **0 known-open** of the **4** UBL `existence` asserts (histogram total **4**).
- Every implemented id is graded in `differential.py` LEG 5 (the `sb` leg) and agrees with the official CEN EN16931-UBL Schematron at **0 divergences**; findings surface under the distinct **`syntax-binding`** report category mirroring the official `@flag`.
...
The single UBL `datatype-regex` assert is UBL-DT-01, `string-length(substring-after(.,'.'))<=2` — a 2-decimal cap on amounts, bound to the element-suffix function context `//*[ends-with(name(),'Amount') and not(ends-with(name(),'PriceAmount')) and not(ancestor::cac:Price/cac:AllowanceCharge)]`. Both the `@test` and the context compile under bounded, provable extensions: the decimal cap is a pure string operation (`substring-after(., '.')` = the part after the FIRST `.`, length ≤ 2 — no rounding, no `number()` coercion), and the context is the existing element-suffix branch grammar plus one bounded ancestor guard `not(ancestor::cac:Price/cac:AllowanceCharge)` (exclude an amount that has a `cac:Price` ancestor carrying a `cac:AllowanceCharge` child). A `matches()` regex restriction would still stay known-open — no regex engine is faked.

- **1 implemented** (differential-proven) / **0 known-open** of the **1** UBL `datatype-regex` asserts (histogram total **1**).
- Every implemented id is graded in `differential.py` LEG 5 (the `sb` leg) and agrees with the official CEN EN16931-UBL Schematron at **0 divergences**; findings surface under the distinct **`syntax-binding`** report category mirroring the official `@flag`.
...
- **510 implemented** (differential-proven) / **18 known-open** of the **528** CII `absence-restriction` asserts (histogram total **528**).
...
- **43 implemented** (differential-proven) / **9 known-open** of the **52** CII `cardinality-count` asserts (histogram total **52**).
- Every implemented id is graded in `differential.py` LEG 6 (the `sbcii` leg) and agrees with the official CEN EN16931-CII Schematron at **0 divergences**; findings surface under the distinct **`syntax-binding`** report category mirroring the official `@flag` (warnings do not change the exit code).
...
The single CII `other-complex` assert (CII-SR-119) is a compound `and`/`or` co-occurrence constraint over a gross-price applied allowance/charge. Its exact semantics stay outside the restricted grammar: the left disjunct carries a predicate `ram:ChargeIndicator[udt:Indicator=false()]` whose truth is an XPath-2.0 typed general comparison against the boolean `false()` (the `udt:Indicator` untyped value is cast to `xs:boolean` — valid only for `true`/`false`/`1`/`0`), conjoined/disjoined with two existence tests over `ram:ActualAmount`. Modelling the cast-to-boolean equivalence AND the `or`(`and`,`and`) shape is not a bounded, provable form here, so per the honesty line it is left machine-listed as known-open — never approximated.
...
Each `@test` is mechanically classified into ONE coarse shape class by
the shape of the expression (NOT by id family — a `*-DT-*` assert whose
test is `not(@schemeName)` is honestly an `absence-restriction`). This
histogram is the artifact that decides implementation batch order: the
two dominant classes cover the overwhelming majority.

| binding | shape | asserts | % of binding |
| --- | --- | ---: | ---: |
| UBL | `absence-restriction` | 699 | 92.5% |
| UBL | `cardinality-count` | 52 | 6.9% |
| UBL | `datatype-regex` | 1 | 0.1% |
| UBL | `existence` | 4 | 0.5% |
| CII | `absence-restriction` | 528 | 90.6% |
| CII | `cardinality-count` | 52 | 8.9% |
| CII | `datatype-regex` | 1 | 0.2% |
| CII | `existence` | 1 | 0.2% |
| CII | `other-complex` | 1 | 0.2% |

Shape classes: `absence-restriction` — the test is a negated presence check — not(...) — asserting a UBL/CII element or attribute MUST NOT appear (the EN 16931 core model has no slot for it). Dominates the *-CR-* family.; `cardinality-count` — the test compares a count(...) of repeated nodes against a bound (e.g. count(x) <= 1) — a cardinality cap. Dominates the *-SR-* family.; `datatype-regex` — the test constrains a leaf value's lexical form — matches(), string-length(), or a substring/number decimal-place check. The *-DT-* decimal/format restrictions.; `existence` — the test asserts a node or attribute simply EXISTS — exists(...), a bare parenthesized location path, an @attr presence, or a normalize-space(...) != '' non-empty check.; `other-complex` — a compound conditional the four coarse shapes above do not capture (e.g. an and/or co-occurrence constraint) — flagged for hand review.
