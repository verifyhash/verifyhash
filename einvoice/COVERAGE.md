...
- Honesty note: these are **NOT** part of the **286 business rules**
  counted above, and the engine evaluates **none** of them. This is a
  measurement + design artifact (catalogued in
  `syntax_binding_catalog.json`, regenerated by `gen_syntax_binding.py`,
  re-parsed live by `test_syntax_binding.py`); no matrix syntax tag is
  flipped and no assert evaluator is implemented here.
