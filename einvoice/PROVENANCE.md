...
| Version | `0.2.9` (source of truth: [`pyproject.toml`](pyproject.toml)) |
| PURL | `pkg:pypi/verifyhash-einvoice@0.2.9` |
| License | Apache-2.0 |
...
| Distribution | **0.2.7 is published and is the release PyPI currently serves; 0.2.9 is prepared and not yet uploaded.** 0.2.7 was uploaded on 2026-07-29 and `info.version` on the JSON API reads `0.2.7`. `0.2.8` was cut on disk on 2026-07-29 and **never uploaded** — no such artifact exists on PyPI — and 0.2.9 folds its licence/storefront payload in, so 0.2.9 is what supersedes 0.2.7 (see [`REPUBLISH-PYPI.md`](REPUBLISH-PYPI.md)). PyPI versions are immutable, so 0.2.7 stays available and uncorrectable forever — which is why its missing in-artifact `LICENSE` can only be fixed by a new upload, never by re-cutting 0.2.7. Installable today from a source checkout / vendored copy, or as 0.2.7 from PyPI. |
