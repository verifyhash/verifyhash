...
BR-DE-15  Buyer reference (BT-10) must be transmitted (non-empty).
  ...
  rule page: https://verifyhash.com/einvoice/rules/BR-DE-15/
  in German: einvoice --explain BR-DE-15 --lang=de
```
...
The last two lines are exits rather than fields. **rule page** is this rule's
page on the site — one of 297, generated by `gen_site.py` from the same
`remediation_catalog.json` this block prints, so a page exists for every id
`--explain` accepts, and it is the identical URL the SARIF `helpUri` and the
HTML report's rule link use. Follow it when you want the English and the German
wording *side by side* (the CLI shows one language at a time) plus the
provenance note in full; the page is self-contained and opens offline. **in
German** is the next command, ready to paste; it disappears once you are
already reading German, and would be omitted for any rule the catalog has no
German for — today that is none of the 297.
...
Two smaller differences from the English block above: `rule page` is unchanged
— that page is framed in English, but it carries the German title and the
German fix next to the English ones, so it is worth following from here too —
and the `in German` pointer is gone, because you are already there.
