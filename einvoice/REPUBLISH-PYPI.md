# REPUBLISH-PYPI — publish `verifyhash-einvoice` 0.2.9 to PyPI (owner/supervisor action, ~10 min)
...
upload: version **0.2.9**, which supersedes 0.2.7. The loop stages this packet
...
**Why 0.2.9 is worth a sitting.** The published 0.2.7 wheel ships **no
...
product is currently non-compliant. 0.2.9 fixes exactly that (plus the
...
installer until 0.2.9 is uploaded. **0.2.9 also changes engine behaviour**, unlike
the never-uploaded 0.2.8 cut it supersedes: the German HTML report now renders
German rule titles and fix hints with per-rule provenance, every HTML report
carries a footer naming the build that wrote it, and `--explain` links the
rule's canonical page. No rule, profile or exit code moved — see `CHANGELOG.md`.
...
runbook publishes 0.2.9.** 0.2.7 is itself immutable now that it is served, so
...
`einvoice/__init__.py` are already at 0.2.9, so this precondition is satisfied
by following the runbook as written: publish 0.2.9, never re-cut 0.2.7. A repeat
...
| Version to publish | `0.2.9` | `pyproject.toml` `[project] version`, matches `einvoice.__version__` (lock-step enforced by `test_packaging.py`) |
...
build step of the command sequence below now emits correctly-named 0.2.9
artifacts. The
...
python3 -m build            # writes dist/verifyhash_einvoice-0.2.9-py3-none-any.whl
                            #    and dist/verifyhash_einvoice-0.2.9.tar.gz
...
bash tools/release-preflight.sh dist/verifyhash_einvoice-0.2.9-py3-none-any.whl
...
#   einvoice --version   # -> einvoice 0.2.9
...
adds 0.2.9 alongside the existing releases; it does not — and cannot — touch the
...
build (see step 1) can reintroduce the 0.2.6 engine under a 0.2.9 label. A
