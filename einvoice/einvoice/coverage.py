...
def _render_syntax_binding(w):
    ...
    w("  counted above, and the engine evaluates **none** of them. This is a")
    w("  measurement + design artifact (catalogued in")
    w("  `syntax_binding_catalog.json`, regenerated by `gen_syntax_binding.py`,")
    w("  re-parsed live by `test_syntax_binding.py`); no matrix syntax tag is")
    w("  flipped and no assert evaluator is implemented here.")
    w("")
