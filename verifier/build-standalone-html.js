...
const PAGE_STYLE = [
  ...
  // T-75.2: an accepted-but-UNPINNED verdict is AMBER (never the green of a pinned accept) + a warn band.
...
// The honest boundary, verbatim on the page (T-66.2 acceptance d; T-74.4 replaced the internal
// "P-3 trust-root" jargon with the plain "a separate trusted timestamp" a stranger can parse).
const BOUNDARY_HTML = [
...
// T-75.2: the honest boundary the pinned sentence above does NOT yet cover — the UNPINNED-signer risk.
// A SEPARATE paragraph (the sentence above is pinned VERBATIM across the docs, so it stays byte-stable);
// this states plainly that WITHOUT a vendor pin an accept is not provenance — anyone's key passes.
const UNPINNED_BOUNDARY_HTML = [
...
function pageBodyText() {
  return [
    ...
    // T-74.4: the PLAIN-ENGLISH lede a layperson reads FIRST — what the page does, in their words,
    // ABOVE the technical note. One array line so the sentence is a contiguous substring of the page.
    ...
    "</div>",
    // T-75.2: reframe the (previously "optional") pin so skipping it reads as "you are trusting whatever
    // key signed this — anyone's key passes", not as a harmless omission.
    ...
    "</section>",
    "",
    // T-74.4: the agent-session demo is an ADVANCED aside, so it sits AFTER the real-packet verify
    // section and stays COLLAPSED behind a native <details> toggle — a layperson's first screen is
    // lede -> sample challenge -> verify-your-packet; the agent demo opts in with one click (no JS
    // needed for the toggle, so the collapse works even with scripting oddities).
